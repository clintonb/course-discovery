...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        course_runs = CourseRun.objects.filter(seats__type__in=(CourseMode.CREDIT, CourseMode.VERIFIED,)).exclude(
            seats__type=CourseMode.AUDIT)
        if options['commit']:
            for course_run in course_runs:
                seat = course_run.seats.create(type=CourseMode.AUDIT, price=0, upgrade_deadline=None)
